...
## Anti Pattern 10: Using an analytic functions over self-joins
Example:
```
SELECT
...
Output:
```
SelfJoin: Instead of using a self-join at line 11, use a window (analytic) function to reduce the number of additional bytes that are generated by the query.
```
